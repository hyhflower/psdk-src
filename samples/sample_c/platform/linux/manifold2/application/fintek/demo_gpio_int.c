...
static uint64_t g_hhit_ppsNewestTriggerLocalTimeMs = 0;
				...
				for (i = 0; i < MAX_GPIO_COUNT - 1; i++) {
					if (idx[i] != INIT_VALUE) {
						...
						if (psdkStat == DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS)
							g_hhit_ppsNewestTriggerLocalTimeMs = timeMs;
						if(ishow)
						{
							ishow=false;
							printf("GPIO%02x: cnt: %d, data: %d, timeMs: %d\n", idx[i], cnt++, data, timeMs);
						}
					}
				}
			}
	...
	for (i = 0; i < MAX_SET && i < argc - 1; ++i) {
		for (j = 1; j < MAX_GPIO_COUNT; ++j) {
			if (m_idx[i][j] != INIT_VALUE) {
				...
				CHECK_RET(_SetGpioIntTriggerMode(m_idx[i][j], eGPIO_Int_Trigger_Rising));
...
T_DjiReturnCode hhit_GetNewestPpsTriggerLocalTimeUs(uint64_t *localTimeUs)
{
    ...
    if (g_hhit_ppsNewestTriggerLocalTimeMs == 0) {
        USER_LOG_WARN("pps have not been triggered.");
        return DJI_ERROR_SYSTEM_MODULE_CODE_BUSY;
    }

    *localTimeUs = (uint64_t) (g_hhit_ppsNewestTriggerLocalTimeMs * 1000);

    return DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS;
}

T_DjiReturnCode hhit_gpio_Init(void)
{
	int nRet = 0;
	const char *chppBuf[]={"name","0x85,5"};
	int argc=2;
	char **argv=(char**)chppBuf;
