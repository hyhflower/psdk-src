...
static void hhit_OutputPinVal(int iPin,int iVal);
...
int main(int argc, char **argv)
{
    ...
    T_DjiTestTimeSyncHandler testTimeSyncHandler = {
        .PpsSignalResponseInit = hhit_gpio_Init,
        .GetNewestPpsTriggerLocalTimeUs = hhit_GetNewestPpsTriggerLocalTimeUs,
    };
    ...
    USER_LOG_DEBUG("hhit-mountPosition:%d",aircraftInfoBaseInfo.mountPosition);
...
static void hhit_gpioConfig()
{
    ...
    snprintf(chaPath,sizeof(chaPath),"/sys/class/gpio/export");
    fp = fopen(chaPath, "w");
    if (fp == NULL)
    {
        USER_LOG_DEBUG("fopen failed!");
        return;
    }

    for(i=0;i<8;i++)
    {
        snprintf(chaBuf,sizeof(chaBuf),"%d",i);
        bufLen=strlen(chaBuf);
        size = fwrite(chaBuf, 1, bufLen, fp);
        if (size != bufLen)
        {
            fclose(fp);
            USER_LOG_DEBUG("write error");
            return;
        }
    }
    
    USER_LOG_DEBUG("%s cnt: %d.",chaPath, i);
...
static void hhit_OutputPinVal(int iPin,int iVal)
{
    ...
    snprintf(chaPath,sizeof(chaPath),"/sys/class/gpio/gpio%d/value",iPin);
    fp = fopen(chaPath, "w");
    if (fp == NULL)
    {
        USER_LOG_DEBUG("fopen failed!");
    ...
    snprintf(chaBuf,sizeof(chaBuf),"%d",iVal);
    bufLen=strlen(chaBuf);
    size = fwrite(chaBuf, 1, bufLen, fp);
    if (size != bufLen)
    {
    ...
    USER_LOG_DEBUG("%s val: %d.",chaPath, iVal);
